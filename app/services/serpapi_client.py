...
_PAGE_CACHE = TTLCache(ttl_seconds=600, max_items=256)


async def _fetch_page(client: httpx.AsyncClient, app_id: int, page: int):
    cache_key = (app_id, page)
    ...
    data = response.json()
    _PAGE_CACHE.set(cache_key, data)
    return data


async def collect_reviews(app_id: int, limit: int = 100, pool_multiplier: int = 2):
    if not SERPAPI_API_KEY:
        raise RuntimeError("SERPAPI_API_KEY is not set")

    target_pool_size = limit * pool_multiplier
    async with httpx.AsyncClient(timeout=15) as client:
        first_page = await _fetch_page(client, app_id, page=1)
        ...
        for page in range(2, pages_needed + 1):
            data = await _fetch_page(client, app_id, page)
